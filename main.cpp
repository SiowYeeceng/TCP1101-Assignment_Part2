...
void board(int row, int column, int zombie, char *random, int alife, int aattack, int *zombdata, int whoseturn)
{
    ...
    for (z = 0; z < zombie; z++)
    {
        ...
        cout << setw(3) << arrowtowhoseturn;
        cout << "Zombie " << z + 1 << ": Life " << setw(3) << *(zombdata + z * 7) << ", Attack " << setw(3) << *(zombdata + z * 7 + 1) << setw(2) << ", Range " << *(zombdata + z * 7 + 2) << endl;
        arrowtowhoseturn = "";
    }
    cout << endl;
}
                ...
                while (!movedone)
                {
                    ...
                    case 'S':
                        if (aattack <= 10)
                        {
                            aattack = 0;
                            nextcom = com;
                            alienstatus = "Alien found a slime, attack decreased by 10\n";
                            break;
                        }
                        else
                        {
